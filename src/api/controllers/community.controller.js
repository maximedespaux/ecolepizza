...
const getPost = async (req, res) => {
    try {
        ...
        res.json({ data: { ...p, answers, has_image: img ? 1 : 0, mine: p.author_user_id === req.user.id } });
    } catch (err) {
...
const updatePost = async (req, res) => {
    try {
        ...
        if (!auteur && !estStaff(req.user)) return res.status(403).json({ message: 'Publication d\'un autre stagiaire.' });
...
const deletePost = async (req, res) => {
    try {
        ...
        const [[p]] = await conn.query(
            ...
            [req.params.id, req.user.organization_id]);
        if (!p) return res.status(404).json({ message: 'Publication introuvable.' });
        if (p.author_user_id !== req.user.id && !estStaff(req.user)) {
...
const deleteAnswer = async (req, res) => {
    try {
        ...
        if (a.user_id !== req.user.id && !estStaff(req.user)) return res.status(403).json({ message: 'Réponse d\'un autre stagiaire.' });
