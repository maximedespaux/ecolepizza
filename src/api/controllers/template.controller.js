...
/* LE RENOMMAGE DE SLUG A ÉTÉ RETIRÉ (2026-09-09), à la demande de l'organisme.
 *
 * Il existait `PUT /:slug/rename`, qui répercutait le nouveau slug partout où il était référencé —
 * parcours, réglage boutique, factures, documents générés, points de rupture, et les slugs enfouis
 * dans du JSON. Une cascade de dix requêtes pour renommer un identifiant.
 *
 * POURQUOI ON L'ENLÈVE. Un slug est un IDENTIFIANT : sa valeur ne veut rien dire, seule sa
 * stabilité compte. Le renommer n'apporte rien qui ne soit déjà couvert par l'intitulé, qui est
 * libre et modifiable, lui — et fait porter à chaque référence le risque d'être oubliée. Une
 * cascade qui rate une table ne se voit pas le jour du renommage : elle se voit des semaines plus
 * tard, sur un document qui ne se génère plus.
 *
 * Le slug se choisit donc À LA CRÉATION et ne bouge plus. Pour en changer, on duplique le modèle
 * sous le slug voulu. */
