 ...
 * l'école, pas le développeur. Le point à compléter est nommé dans `destinataires`.
 */
...
const FINALITES = {
    partenaires: {
        ...
        /* NOMMER LES DESTINATAIRES est ce qui rend le consentement « éclairé ». Une catégorie vague
           (« nos partenaires ») ne permet pas de savoir à quoi l'on dit oui — et un partenaire
           ajouté plus tard ne serait couvert par aucun accord passé, puisque la personne ne pouvait
           pas le connaître. Le texte réellement affiché est figé dans chaque réponse. */
        destinataires: 'Fournisseurs et partenaires référencés par l\'école — À COMPLÉTER PAR '
            + 'L\'ORGANISME : citer les partenaires ou, à défaut, leurs catégories (matériel, '
            + 'farine, four…)',
        formulation:
...
async function etatCourant(conn, orgId, learnerId) {
    try {
        ...
        return FINALITES_CONNUES.map((k) => ({
            ...
            destinataires: FINALITES[k].destinataires,
    ...
    try {
        await conn.query(
            ...
            [orgId, learnerId, finalite, accorde ? 1 : 0,
             f.destinataires.slice(0, 500), f.formulation.slice(0, 600),
...
module.exports = { FINALITES, FINALITES_CONNUES, etatCourant, enregistrer, isMissingSchema };
