/**
 * « DERNIERS DOSSIERS » MÈNE À LA FICHE DU STAGIAIRE, OUVERTE SUR CE DOSSIER (2026-09-21).
 *
 * LA DEMANDE. Une ligne du tableau de bord ne s'ouvrait pas : pour voir le dossier, il fallait
 * passer par la liste des stagiaires et chercher le nom.
 *
...
test('le lien désigne la fiche ET le dossier', async () => {
    ...
    assert.strictEqual(lienDossier({ id: 'e-42', learner_id: 'l-7' }), '/stagiaires/l-7?dossier=e-42');
    // Un identifiant ne casse pas l'adresse, quoi qu'il contienne.
    assert.strictEqual(lienDossier({ id: 'a&b', learner_id: 'x/y' }), '/stagiaires/x%2Fy?dossier=a%26b');
});
...
test('tableau de bord : chaque ligne mène à la fiche, quand la fiche s\'ouvrira', () => {
    ...
    assert.match(TABLEAU, /\{ficheOuvrable \? \(\s*<Link to=\{lienDossier\(e\)\} className="rowlink dossier-lien"/,
