...
test('LA FEUILLE : le lieu de la SESSION, la déclaration d\'activité, l\'employeur', () => {
    ...
    assert.match(html, /Lieu : Centre de Bordeaux, 12 rue des Fours, 33000 BORDEAUX/, 'et plus l\'adresse de l\'organisme');
    assert.match(html, /Déclaration d'activité n° 76 65 00989 65/);
    assert.match(html, /Entreprise : <b>Le Four à Bois SAS<\/b>/);
    assert.match(feuille({ lieu: null }), /Lieu : 101 rue Alsace Lorraine, 65300 LANNEMEZAN/, 'sans lieu de session : l\'adresse de l\'organisme');
});
...
test('LA FEUILLE : le rattrapage imprime son motif, et la présence compte dans le total', () => {
    ...
    assert.match(html, />35h00<\/td>/, 'le volume prévu, en bout de ligne');
});
...
test('LA FEUILLE : les horaires ne s\'impriment qu\'une fois ; le soir sous « Après-midi »', () => {
    const html = feuille();
    assert.doesNotMatch(html, /Horaires : Jour 1/, 'la grille les montre déjà, ligne « Horaires »');
    assert.match(feuille({ config: { slots: ['MATIN', 'APRES_MIDI'], show_hours: false } }), /Horaires : Jour 1/, 'sans la ligne de la grille, l\'en-tête les garde');
    // Le cours du soir : sa plage dans la colonne de l'après-midi, rien sous le matin.
    const soir = feuille({ horaires: '17h00 - 19h00' });
    const ligneHoraires = soir.slice(soir.indexOf('>Horaires</td>'), soir.indexOf('</tr>', soir.indexOf('>Horaires</td>')));
    const cases = [...ligneHoraires.matchAll(/<td width="\d+">([^<]*)<\/td>/g)].map((m) => m[1]);
    assert.deepStrictEqual(cases.slice(0, 2), ['', '17h00 - 19h00']);
});

test('LA FEUILLE : les noms à gauche (l\'attribut, que LibreOffice lit), le formateur avec son rôle', () => {
    const html = feuille();
    assert.match(html, /<td class="nm" align="left"[^>]*>BERGER Camille<div class="sub">Stagiaire<\/div><\/td>/);
    assert.match(html, /<td class="nm" align="left"[^>]*>DESPAUX Jean-Jacques<div class="sub">Formateur<\/div><\/td>/);
});
...
test('LE CHARGEUR : une présence RATTRAPÉE sans signature compte, et son motif s\'imprime', async () => {
    ...
    assert.match(html, /Rattrapage : Oubli de signature \(Jean-Jacques DESPAUX\)/);
