...
test('LES JETONS SONT DANS LA PALETTE — sinon ils existent sans que personne puisse les insérer', () => {
    /* DÉFAUT MESURÉ EN PRODUCTION, juste après la mise en ligne. LA PALETTE N'EST PAS LE
       ...
       justes séparément. */
    for (const groupe of ['Évaluation pratique', 'Jury']) {
        assert.ok(PALETTE.includes(`groups.push(catalogGroup('${groupe}'))`),
            `${groupe} doit être poussé dans la palette`);
        const ordre = /GROUP_ORDER = \[([\s\S]*?)\];/.exec(PALETTE);
        assert.ok(ordre && ordre[1].includes(`'${groupe}'`),
            `${groupe} doit être rangé à sa place, sinon il tombe en fin de liste`);
    }
...
test('l\'ordre des jetons d\'évaluation n\'est pas trié alphabétiquement', () => {
    ...
    const m = /CURATED_GROUPS = new Set\(\[([^\]]*)\]\)/.exec(PALETTE);
    for (const groupe of ['Évaluation pratique', 'Jury']) {
        assert.ok(m && m[1].includes(`'${groupe}'`), `l'ordre du groupe ${groupe} est réfléchi`);
    }
});
...
test('une évaluation ne s\'imprime pas sur un document de GROUPE', () => {
    ...
    for (const groupe of ['Évaluation pratique', 'Jury']) {
        const m = /HIDDEN_FOR_COMPANY = new Set\(\[([^\]]*)\]\)/.exec(PALETTE);
        assert.ok(m && m[1].includes(`'${groupe}'`), `${groupe} doit être masqué sur un document de groupe`);
    }
});
