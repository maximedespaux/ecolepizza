...
export default function ConsentModal() {
  ...
  useEffect(() => {
    ...
    getMyConsents().then((r) => {
      ...
      const premiere = liste.find((f) => f.accorde === null);
  ...
  const repondre = async (accorde) => {
    setBusy(true); setErreur(null);
    try {
      await setMyConsent(aDemander.cle, accorde);
      setADemander(null);
      ...
      <div className="modal consent-modal" onClick={(e) => e.stopPropagation()}>
        <div className="mhead">
          <h3 style={{ fontSize: 16, display: "flex", alignItems: "center", gap: 8 }}>
            <Icon name="shield" size={17} /> {aDemander.titre}
          </h3>
        ...
        <div className="mfoot consent-foot">
          {/* MÊME POIDS VISUEL pour les deux réponses : un refus mis en retrait est un choix guidé. */}
          <button className="btn consent-choix" disabled={busy} onClick={() => repondre(false)}>
            Je refuse
          </button>
