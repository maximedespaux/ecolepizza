...
function Parametres() {
  ...
  return (
    <>
      ...
      />
      {items.length === 0 ? (
        <Card><EmptyState icon="settings">Aucun paramètre accessible.</EmptyState></Card>
      ) : (
        <div className="grid cols-3">
          {items.map((it) => (
            <Link key={it.to} to={it.to} className="card param-card"
              ...
              <span style={{ width: 34, display: "grid", placeItems: "center", flex: "0 0 34px", color: "var(--blue)" }}><Icon name={it.ic} size={22} /></span>
