...
export default function RealisationWizard() {
  ...
  function saveBuild({ asNew = false, ...overrides } = {}) {
    // Sans empâtement choisi il n'y a rien à enregistrer : on le DIT, au lieu de ne rien faire.
    if (!emp) { setEchec((n) => n + 1); return; }
    const nom = (overrides.name ?? r.name ?? "").trim() || (garn ? garn.name : "Ma réalisation");
    overrides.name = nom;
    setR((p) => ({ ...p, name: nom }));
